...
function startgraph() {
  let chosenCountry = d3.select("#selectmenu").property("value");
  ...
  const graphTop = svgTop
    .append("g") // groupe avec tout les battonet dedans
    .attr("widht", graphW)
    .attr("height", graphH)
    .attr("transform", `translate(${margin.left},${margin.top})`);

  const groupeX = graphTop.append("g").attr("transform", `translate(0,450)`); // translate de 450, la ou doit etre l'axis x

  const groupeY = graphTop.append("g");

  const rectanglesTop = graphTop
    .selectAll("rect")
    .data(dataCountryOne)
    .enter()
    .append("rect")
  ...
  const axeY = d3
    .axisLeft(y) // création de l'axe pour les valeurs de crimes
    .ticks(20)
    .tickFormat((d) => d + " (en milliers)");

  groupeX
    .call(axeX) // appel de l'axe des dates dans le grp X
    .style("font-size", "14px");
  groupeY
    .call(axeY) //appel de l'axe des valeurs dans le grp Y
...
function startgraphbottom() {
  let chosenCountryBottom = d3.select("#selectmenuBottom").property("value");
  ...
  const graphBottom = svgBottom
    .append("g") // groupe avec tout les battonet dedans
    .attr("widht", graphWB)
    .attr("height", graphHB)
    .attr("transform", `translate(${marginB.left},${marginB.top})`);

  const groupeXB = graphBottom.append("g").attr("transform", `translate(0,450)`); // translate de 450, la ou doit etre l'axis x

  const groupeYB = graphBottom.append("g");

  const rectanglesBottom = graphBottom
    .selectAll("rect")
    .data(dataCountryTwo)
    .enter()
    .append("rect")
  ...
  const axeYB = d3
    .axisLeft(yB) // création de l'axe pour les valeurs de crimes
    .ticks(20)
    .tickFormat((d) => d + " (en milliers)");

  groupeXB
    .call(axeXB) // appel de l'axe des dates dans le grp X
    .style("font-size", "14px");
  groupeYB
    .call(axeYB) //appel de l'axe des valeurs dans le grp Y
    .style("font-size", "11px");
